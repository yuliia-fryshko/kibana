...
import { fetchTraceSampleIds, fetchExitSpanSamples } from './apm_data';
...
async function buildTopologyFromTraceIds({
  ...
  const spans = await fetchExitSpanSamples({
    apmEventClient: resources.apmEventClient,
    traceIds,
    start: startMs,
    end: endMs,
  });
...
async function getDownstreamTopology({
  ...
  if (maxDepth === 1) {
    ...
    return getImmediateDownstreamDependencies({
      apmEventClient: resources.apmEventClient,
      randomSamplerSeed: resources.randomSamplerSeed,
      serviceName,
      startMs,
      endMs,
    });
  ...
  const { traceIds } = await fetchTraceSampleIds({
    apmEventClient: resources.apmEventClient,
...
export async function getServiceTopology({
  ...
  const resources: TopologyResources = { apmEventClient, randomSamplerSeed, config, logger };
